...
int num=((int) n);
sem_wait(&writer);
printf("\nwriter %d entered the block",num);
data=data*2;
printf("The Data is written by writer %d",data,num);
sleep (2);
sem_post(&writer);
}
void *rfunc(void * n)
...
if (readers==1)
{
printf ("READER %d Blocked The writers",num);
sem_wait(&writer);
}
sem_post(&reader);
printf("Reader %d read data-> %d",num,data);
sleep(2);
sem_wait(&reader);
readers--;
if(readers==0)
